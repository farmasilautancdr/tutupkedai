...
- **Google Drive sync (`api/outlet.js`, `GET`/`POST /api/outlet?code=<code>`):** frontend calls this from `chooseOutlet`, `loadOutletData`, and every place that mutates `outletConfig`/`scanHistory` (see `syncOutletToDrive`/`fetchOutletFromDrive` in `index.html`). Auth is a Google **service account** (JWT-signed by hand in `api/outlet.js`, no `googleapis` dependency, no per-user login). All outlets live inside **one combined file**, `tutupkedai-data.json`, in the shared Drive folder — **not** one file per outlet.
  ...
  - `VALID_CODES` (now `api/lib/outletCodes.js`, shared by `api/outlet.js` and `api/sheet.js`) mirrors `allOutlets` in `index.html` — keep both lists in sync if outlets are added/removed.
- **Google Sheet export (`api/sheet.js`, `POST /api/sheet?code=<code>`):** manual "Save to Sheet" button per outlet (next to Export CSV/PDF) — separate from the automatic JSON sync above. Writes that outlet's scan history into a pre-created Google Sheet file named exactly by outlet code, split into **one tab per calendar month** (title `"Aug 2026"`, `"Sep 2026"`, etc. — `groupEntriesByMonth` in `api/lib/sheetRows.js`; entries with no parseable date land in an `"Undated"` tab). Missing month tabs are auto-created (`spreadsheets.batchUpdate` `addSheet`) — **not** subject to the Drive quota wall below, since that only blocks creating a new Sheet *file*, not a new tab inside one the service account already has Editor on. Tabs whose title matches this export's own "Mon YYYY"/"Undated" naming (`isMonthTabTitle` in `api/lib/sheetRows.js`) but no longer have any matching entries in the freshly-posted `scanHistory` are auto-deleted (`planTabSync`), so month tabs don't pile up stale after data is reset/edited — but if deleting every stale tab would leave the spreadsheet with zero sheets (Sheets forbids that), the last one is kept and cleared to header-only instead of deleted. Any tab **not** matching that naming (e.g. a leftover default `Sheet1`, or an old pre-monthly-tabs flat sheet a human made) is never touched, created, or deleted. Each tab: sorted by date, same-date rows merged in the Date column, a Daily Summary row every `posCount * 2` receipts — merged across the full row width (`MERGE_ALL`, not just text overflowing into blank neighbor cells) and word-wrapped, currency formatting on amount columns. Same service-account auth as `api/outlet.js` (shared via `api/lib/googleAuth.js`), extended with the `spreadsheets` scope. Same Drive quota wall as the JSON file for the Sheet *file* itself — must be pre-created once (`scripts/create-outlet-sheets.js`, run under your own Google login) and shared with the service account as Editor; the endpoint only ever finds-and-updates an existing Sheet file. Row-building/sorting/merge/month-grouping logic lives in `api/lib/sheetRows.js` (unit tested via `node --test`).
  - **The Google Sheets API must be manually enabled** on the Google Cloud project the service account belongs to — separate from the Drive API, which was already on. Discovered live: first real calls failed with a `403 SERVICE_DISABLED` until enabled at `console.developers.google.com/apis/api/sheets.googleapis.com/overview?project=<project-number>`; takes a few minutes to propagate after enabling.
  - `scripts/create-outlet-sheets.js`'s OAuth consent screen is in Google's "Testing" publishing status — only the Google account(s) added as Test users can complete its login flow (`access_denied` otherwise). Adding a brand-new outlet later means re-running that script for just that code (`--outlet=<CODE>`), which needs this same login again.
- **Deployment:** Vercel project `tutupkedai` under the `farmasilautancdr` org, deployed from `origin/main` via GitHub import (no build step). Live at `https://tutupkedai.vercel.app` — that's the stable domain to hand to staff, not the random per-deployment `tutupkedai-<hash>-farmasilautancdr-...vercel.app` URLs (those change every deploy). **Deployment Protection (Vercel Authentication/SSO) must stay OFF for Production** — it's on by default for team-owned projects and would lock staff out entirely; the app has no login of its own.
